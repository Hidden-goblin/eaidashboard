...
class MapHtml(OutputStrategy):
    ...
    async def render(table_rows):
        ...
        filename = BASE_DIR / "static" / f"testoutput_{uuid.uuid4()}.html"
        output_file(filename=filename, title="Status map over time")
        TOOLS = "hover,save,pan,box_zoom,reset,wheel_zoom"
        p = figure(title="Test map result",
        ...
        p.rect(x="run_date",
               y="element_name", width=1, height=1,
               source=_json,
...
class MapCsv(OutputStrategy):
    ...
    async def render(table_rows):
        ...
        with open(filename,
                  ...
                  newline="") as file:
            _csv = csv.writer(file, quoting=csv.QUOTE_ALL)
            _csv.writerow(("run_date", "element_id", "status", "element_name"))
            _csv.writerows(table_rows)
        return filename.name
