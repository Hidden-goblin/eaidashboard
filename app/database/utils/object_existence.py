...
async def project_version_exists(project_name: str, version: str = None) -> ApplicationError:
    """:raise ProjectNotRegistered
       :raise VersionNotFound
    """
    if provide(project_name) is None:
        return ApplicationError(error=ApplicationErrorCode.project_not_registered,
                                message=f"'{project_name}' is not registered")
