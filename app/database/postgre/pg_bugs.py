...
async def get_bugs(
    ...
    final_query = (
        f"{query} where {' and '.join(query_filter)}"
...
async def db_update_bugs(
    ...
    current_bug: BugTicketFull = await db_get_bug(
        project_name,
        internal_id,
    )
...
async def make_link_to_scenario(
    ...
    # Insert
    query = (
        "insert into bugs_issues (bug_id, occurrence, ticket_reference, scenario_id)"
        ...
        with pool.connection() as connection:
            ...
            with connection.cursor() as cursor:
                ...
                if len(data) != cursor.rowcount:
                    raise Exception(
                        f"linked {cursor.rowcount} scenarios to bug {bug_id} while expecting {len(data)}"
