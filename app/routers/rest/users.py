...
@router.put(
    ...
    responses={401: {"model": ErrorMessage, "description": "Cannot authenticate the user"}},
)
async def update_me(
    body: UpdateMe,
    user: User = Security(
...
@router.delete(
    ...
    responses={
        401: {"model": ErrorMessage, "description": "Cannot authenticate the user"},
        400: {"model": ErrorMessage, "description": "One of the delete business rule is not met"},
    },
)
...
@router.get(
    "/projects/{project_name}/users",
    tags=["Users", "Projects"],
    description="""Get users linked to a projects""",
    response_model=List[UserLight | str],
)
async def all_users_in_project(
    project_name: str,
    response: Response,
    limit: int = 10,
    skip: int = 0,
