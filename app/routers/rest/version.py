...
@router.post(
    ...
    responses={
        404: {"model": ErrorMessage, "description": "Project name is not registered (ignore case)"},
...
async def create_version(
    ...
    try:
        result = await create_project_version(
            project_name,
            project,
        )
...
@router.get(
    "/projects/{project_name}/versions/{version}",
...
async def update_version(
    ...
    await project_version_exists(
        project_name,
        version,
    )
    try:
        result = await update_version_data(
            project_name.casefold(),
            version.casefold(),
            body,
        )
