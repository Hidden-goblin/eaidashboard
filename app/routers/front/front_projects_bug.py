...
def __validate_from_scenario(
    version: str = None,
    occurrence: int = None,
    ticket_id: int = None,
    scenario_id: int = None,
) -> bool:
    return version is not None and occurrence is not None and ticket_id is not None and scenario_id is not None
    ...
    try:
        ...
        if requested_item is None:
            ...
            if __validate_from_scenario(
                ...
                await db_set_campaign_ticket_scenario_status(
                    project_name,
                    version,
                    occurrence,
                    ticket_reference,
            ...
            return templates.TemplateResponse(
                "forms/add_bug.html",
                {
                    ...
                    "project_name": project_name,
                    "project_name_alias": provide(project_name),
                    "criticality": [f"{crit}" for crit in BugCriticalityEnum],
                    "test": test,
                },
                headers={
                    "hx-retarget": "#modals-here",
                },
            )
...
@router.get(
    "/{project_name}/bugs/{internal_id}",
    tags=["Front - Project"],
    include_in_schema=False,
)
async def display_bug(
    project_name: str,
    internal_id: str,
    request: Request,
    user: User = Security(front_authorize, scopes=["admin", "user"]),
    ...
    if not isinstance(user, (User, UserLight)):
        return user
    try:
        bug = await db_get_bug(
            project_name,
            internal_id,
        )
        versions = await get_versions(project_name)
        return templates.TemplateResponse(
            "forms/update_bug.html",
            {
                "request": request,
                "project_name": project_name,
                "project_name_alias": provide(project_name),
    ...
    try:
        ...
        return templates.TemplateResponse(
            "forms/update_bug.html",
            {
                ...
                "bug": posted,
                "message": (
                    "The update cannot be done. Ensure that the bug title is not "
...
async def front_update_bug_patch(
    project_name: str,
    internal_id: str,
    body: dict,
    request: Request,
