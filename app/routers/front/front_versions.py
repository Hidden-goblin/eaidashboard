...
async def project_versions(
    ...
    try:
        ...
        if request.headers.get("eaid-request", "") == "table":
            ...
            versions.extend([{"value": item["version"], "status": item["status"]} for item in _versions["current"]])
            versions.extend([{"value": item["version"], "status": item["status"]} for item in _versions["archived"]])
            return templates.TemplateResponse(
                "tables/project_version.html",
                {
                    "request": request,
                    "versions": versions,
                    "project_name": project_name,
...
@router.get(
    "/{project_name}/versions/{version}",
    tags=["Front - Project"],
    include_in_schema=False,
)
async def project_version_tickets(
    project_name: str,
    version: str,
    request: Request,
    user: User = Security(front_authorize, scopes=["admin", "user"]),
    ...
    try:
        ...
        if request.headers.get("eaid-request", "") == "versionUpdate":
            ...
            return templates.TemplateResponse(
                "forms/update_version_modal.html",
                {
                    "request": request,
                    "project_name": project_name,
                    "version": version,
...
async def add_ticket_to_version(
    project_name: str,
    version: str,
    request: Request,
    body: dict,
    ...
    try:
        ...
        if not isinstance(result, RegisterVersionResponse):
...
@router.put("/{project_name}/versions/{version}", tags=["Front - Project"], include_in_schema=False)
async def project_version_update(
    project_name: str,
    version: str,
    body: dict,
    request: Request,
    ...
    try:
        ...
        await update_version_data(
            project_name,
            version.version,
            UpdateVersion(**cleaned_body),
        )

        return templates.TemplateResponse(
            "void.html",
            {"request": request},
            headers={"HX-Trigger": "modalClear", "HX-Trigger-After-Swap": "update-dashboard"},
        )
...
async def add_version(
    ...
    try:
        ...
        return templates.TemplateResponse(
            "void.html", {"request": request}, headers={"HX-Trigger": "update-version-table"}
        )
    except Exception as exception:
        log_error(repr(exception))
        return front_error_message(templates, request, exception)
...
@router.get(
    "front/v1/projects/{project}/versions/{version}",
    tags=["Front - Campaign"],
    include_in_schema=False,
)
async def get_project_version(
    ...
    try:
        ...
        log_message(repr(version))
        return templates.TemplateResponse(
            "forms/update_version_modal.html", {"request": request, "version": repr(version)}
        )
    except Exception as exception:
        return front_error_message(templates, request, exception)
