...
class TestRestProjects:
    ...
    def test_update_version_errors_401(
        ...
        assert response.status_code == 200
        response = application.post("/api/v1/projects/test/versions", json={"version": "1.0.2"}, headers=logged)
        assert response.status_code == 200
        response = application.put(
            "/api/v1/projects/test/versions/1.0.0",
    ...
    def test_update_versions_transition(
        ...
        response = application.put("/api/v1/projects/test/versions/1.0.2", json={"status": status}, headers=logged)
        assert response.status_code == 200
