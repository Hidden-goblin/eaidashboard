...
class TestRestCampaignWorkflow:
    ...
    project_version = {"past": "0.9",
                       "current": "1.0",
                       "next": "1.1"}
    dev_tickets = {"first": {"reference": "ref-001",
                             "description": "Description of first"},
                   "second": {"reference": "ref-002",
                              "description": "Description of second"},
                   "third": {"reference": "ref-003",
    ...
    def test_setup(self, application, logged):
        ...
        response = application.post("/api/v1/settings/projects",
                                    json={"name": TestRestCampaignWorkflow.project_name},
                                    headers=logged)
        assert response.status_code == 200

        # Create versions
        for ver in TestRestCampaignWorkflow.project_version.values():
            response = application.post(f"/api/v1/projects/{TestRestCampaignWorkflow.project_name}/versions",
                                        json={"version": ver},
                                        headers=logged)
            assert response.status_code == 200
        ...
        with open("tests/resources/repository_as_csv.csv", "rb") as file:
            response = application.post(f"/api/v1/projects/{TestRestCampaignWorkflow.project_name}/repository",
                                        files={'file': file},
                                        headers=logged)
        ...
        response = application.post(f"/api/v1/projects/{TestRestCampaignWorkflow.project_name}/campaigns",
                                    json={"version": TestRestCampaignWorkflow.project_version['current']},
                                    headers=logged)
        assert response.status_code == 200

        # Add ticket in campaign
        response = application.put(f"/api/v1/projects/{TestRestCampaignWorkflow.project_name}/campaigns"
                                   f"/{TestRestCampaignWorkflow.project_version['current']}/1",
                                   json={"ticket_reference": "ref-001",
                                         "scenarios": [{"epic": "first_epic",
        ...
        response = application.put(f"/api/v1/projects/{TestRestCampaignWorkflow.project_name}/campaigns"
                                   f"/{TestRestCampaignWorkflow.project_version['current']}/1",
                                   json={"ticket_reference": "ref-002",
                                         "scenarios": [{"epic": "second_epic",
                                                        "feature_name": "Test feature",
    ...
    def test_retrieve_campaign(self, application, logged):
        response = application.get(f"/api/v1/projects/{TestRestCampaignWorkflow.project_name}/campaigns"
    ...
    def test_workflow_test_bug_link(self, application, logged):
        response = application.get(f"/api/v1/projects/{TestRestCampaignWorkflow.project_name}/campaigns"
